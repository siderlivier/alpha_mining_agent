...
| 測試 | **226 passed**，含四套前瞻偏差檢測，每套都做過變異驗證 |
...
對上因子庫時就先死了。條件讀法下 **S4 才是最狠的一關**，走到那裡的人 89% 被刷掉。

失因分類：`hypothesis_wrong` 396 筆、`expression_bad` 300 筆、`duplicate` 269 筆。
...
這是一個**可否證**的主張：如果 agent 真的在退步，S1 或 S4 的淘汰率應該上升。
`mining_stats.py` 會直接印出判讀，走向不符時會說「假設品質下降」而不是「飽和」。
...
這也是成本槓桿所在：降低 `candidates_per_round` 比縮減 prompt 有效得多。
...
所以這個比較問的是最赤裸的問題：**LLM 寫的公式，比得過它用的原始食材嗎？**
...
> ⚠️ **一個必須說出來的不對稱**：自有因子的中位覆蓋是 4 個產業，基準是 6 個。
> 自有因子是在自己的主場被量的。這是 `approved_groups` 機制的本意
> （只在有效的地方核准），但它讓比較不是純粹的蘋果對蘋果。
...
| 指標 | 自有勝過基準 | 中位差 |
|---|---|---|
| 平均 IC 較高 | 51.8% | +0.0015 |
| **ICIR 較高** | **63.6%** | **+0.082** |
| **全距較小（較穩）** | **67.8%** | **−0.117** |

**平均 IC 幾乎是五五波，但 ICIR 贏 63.6%、全距較小贏 67.8%。**

也就是說：**LLM 挖出來的公式，相對於原始欄位，主要貢獻是「把同樣強度的訊號變得比較不抖」。**
這其實合理——那些公式大多在做正規化、去噪、取穩定度
（`neg(ts_std(...))`、`sdiv(x, ts_std(x))`、`cs_rank` 交叉相乘）。

典型樣本是 **F-009 營收加速趨勢**：平均 IC 比基準**低** 0.013，但全距小了 0.427、
ICIR 高 0.235——訊號沒比較強，但穩定非常多。

而底部五個（F-011、F-013、F-003、F-008、F-001）**連穩定度都輸給原始欄位**，
F-011 生技轉骨訊號三項全輸。26 個裡有 5 個打不贏基準，這件事列出來比藏起來可信。

> 符號慣例：`d_ic_range` 是唯一「正值代表較差」的欄位（自有因子波動較大）。
...
```bash
python -B src/ml_diagnose.py --liquidity --set own --model lgbm
```

| 因子組 | 持股流動性百分位 | 低流動貢獻 | 中流動 | 高流動 |
|---|---|---|---|---|
| **自有** | **0.45** | **36.0%** | 36.1% | 27.8% |
| 基準 | **0.52** | 26.6% | 32.8% | **40.6%** |

**偏小型股的是自有因子，不是基準因子**（0.5 = 市場中位）。
這對自有因子是壞消息的一半：它的 edge 有一部分在比較難交易的股票上。
...
> 那是另一種汙染（研究者自由度），任何統計校正都修不了，只能靠真的往前走。
...
### 6.4 選擇偏誤：walk-forward 保護的是參數，不是「選哪些因子」

用 test 期的指標挑因子、再用 test 期當成績，就是拿答案卷挑答案。
`factor_lab --greedy` 因此拆成 `--select-span`（在哪裡挑）與 `--span`（在哪裡報）：

| 做法 | 挑選期 | test 期 IR |
|---|---|---|
| **全部 26 個，不挑** | — | **1.583** |
| 貪婪挑 k=3 | validation | 1.425 |
| ~~貪婪挑 k=10~~ | ~~test~~ | ~~2.264~~ ← 假的 |

挑出來的組合在 validation 期 IR 高達 4.406，到 test 期只剩 1.425——**那 4.4 是 24 個月樣本上的噪音。**
...
python -B src/ml_diagnose.py --liquidity --set own
...
src/mining_stats.py             漏斗組成與「飽和 vs 退步」的可否證判讀
