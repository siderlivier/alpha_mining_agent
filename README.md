...
- **所有實測數據與量測指令存檔見 `RESULTS.md`**（含 HMM 的負面結論）
...
python -m pytest tests/ -q    # 應全數通過（建好資料後 217 passed，約 2 分鐘）
```
...
```bash
python src/hmm_regime.py --ablate            # 逐一比較各特徵組合
python src/hmm_regime.py --ablate --seeds 0,1,2,3,4
```

| 特徵組合 | 看多月數 | IR | 超額 α | beta |
|---|---|---|---|---|
| **靜態：全部因子（不切換）** | — | **1.74** | 13.27% | 0.932 |
| 純價格（對照組） | 42 | 1.08 | 13.52% | 0.791 |
| ＋景氣對策信號分數 | 40 | 0.85 | 11.59% | 0.771 |
| ＋景氣燈號 | 45 | 0.88 | 11.87% | 0.770 |
| ＋領先指標年增率 | 51 | 0.78 | 11.02% | 0.796 |
| ＋M1B−M2 交叉 | 75 | 1.15 | 12.07% | 0.862 |
| ＋全部總經 | 42 | 0.67 | 10.61% | 0.762 |
| 只用總經（無價格） | 72 | 0.92 | 10.66% | 0.844 |
| 完美預知（上限） | 111 | 1.98 | 16.97% | 0.874 |

**結論：沒有任何組合勝過「完全不切換」。** 而且加了總經特徵之後，
大多數組合比純價格版**更差**（1.08 → 0.85 / 0.88 / 0.78 / 0.67）。
唯一略好的是 M1B−M2 交叉（1.15）。

每組跑 3 個 seed 取中位數並附全距——Baum-Welch 的初始化會影響結果，
單一 seed 分不出「真的有改善」與「初始化運氣好」。

### 為什麼失敗：模型太保守，而且不是門檻的問題

完美預知說 111 個月該看多，但所有 HMM 變體只喊 39~75 個。
自然的懷疑是「門檻 0.5 太嚴」，但掃過門檻之後發現不是：
...
放寬到 0.1 也只有 58 個月，離 111 還很遠，IR 最高 1.34 仍輸基準 1.74。

原因藏在 `p_bull` 的分布裡：**中位數 0.112、四分位是 0.012 與 0.964**。
模型幾乎總是「非常確定」——110 個月裡有 35 個 p_bull > 0.9、52 個 < 0.1。
它不是猶豫不決，是**很有把握地判斷 47% 的月份為空頭**，而實際只有 35% 是。
調門檻救不了系統性的過度悲觀。
...
mining.log                      逐輪執行紀錄
tests/                          217 個測試（DSL、全管線、因子合成三套前瞻檢測 + 過擬合統計量驗證）
```
