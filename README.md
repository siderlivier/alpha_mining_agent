...
F-026 那一列後來被**撤銷**：資料修復的 migration（`20260919_234830_priority_repairs`）
重算後 train ICIR 掉到 0.557，低於 0.60 門檻。撤銷的理由連同完整指標寫進該因子的
`data_correction_history`，不是靜默移除。最終 5 個因子擴張，其餘維持原範圍。

這 6 次判定全部只用 sub_train + validation，test 沒有參與。
