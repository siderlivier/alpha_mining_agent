...
> 更新日期：2026-09-04（HMM 以 Sharpe 重新檢定後達標；README 待重寫）
...
## 🟢 HMM 市場狀態模型：**達標了 —— 但只在 Sharpe 這個裁判下，且不顯著**

> 2026-09-04 更新：這一節原本寫著「結案，結論是負面的」。
> 那個結論**是用錯指標得出的**，已推翻。完整證據見 `RESULTS.md` 第 16 節。

`python src/hmm_regime.py --riskadj` 的結果（test 期 77 個月，純價格特徵）：

| | beta | alpha | CAGR | **Sharpe** | IR | MaxDD |
|---|---|---|---|---|---|---|
| 靜態：全部因子（不切換） | 0.932 | +13.27% | 39.32% | **1.79** | 1.74 | −15.59% |
| **HMM 切換** | 0.791 | +13.52% | 35.80% | **1.88** | 1.08 | −13.16% |
| 完美預知（上限） | 0.874 | +16.97% | 42.82% | **2.01** | 1.98 | −13.16% |

**1.79 < 1.88 < 2.01 —— 落在驗收區間內，通過。**

之前判它失敗是因為看 IR。IR 是相對基準的指標，切換的本質是降 beta，
在大盤年化 +24.26% 的測試期裡，降 beta 一定被 IR 扣分，**即使風險調整後是賺的**。
把切換版加槓桿 1.178 倍拉回同樣的 beta 之後，它每一欄都不輸靜態：
CAGR 42.86%（+3.54pp）、IR 1.80（+0.06）、MaxDD −15.50%（持平）。

**但是**（這三點必須跟上面那句話一起講，不能只講好的）：

1. **不顯著。** 配對自助法 10000 次，ΔSharpe = −0.091，
   95% CI **[−0.347, +0.152] 跨越 0**，P(靜態較好) = 0.232。
   逐月報酬差配對 t = 1.34。77 個月分不出 1.79 與 1.88。
   → 只能寫「沒有證據顯示切換較差」，不能寫「切換較好」。
2. **MaxDD 的好處是假的。** −13.16% 看起來比 −15.59% 好，
   但等 beta 之後變回 −15.50%。那是曝險低造成的，不是擇時擋掉了什麼。
   想要那個回檔，把靜態組合縮到 0.79 倍就有了，不需要 HMM。
3. **等 beta 需要真的借錢。** 1.178 倍槓桿，這裡假設融資成本 0。
   以 3% 計約吃掉 0.53pp/年（CAGR 42.86% → ~42.3%，仍勝過 39.32%）。

### 總經特徵：這個結論沒有被推翻，而且證據更強了

`--ablate` 也改成用 Sharpe 排序後重跑（`logs/ablate_sharpe.txt`）：

- 純價格 **1.88**，seed 間全距 **0.00** —— 唯一穩健勝過基準的，不是初始化運氣
- ＋景氣燈號 1.84、＋景氣分數 1.82、＋M1B−M2 1.78、＋全部總經 1.76、只用總經 1.68
- **每一個加總經的版本都比純價格差，加得越多越差**，且 seed 全距 ±0.03 比它們與基準的差距還大

> 總經特徵不但沒幫上忙，還**稀釋了價格特徵的訊號**。這是唯一在 IR 與 Sharpe
> 兩個裁判下都成立的原始結論。

### 若要繼續，該做什麼

- [ ] **不要在 test 期上調門檻或狀態數**去把 1.88 推高——那是在測試集上選模型。
      目前的 0.5 門檻與 2 狀態都是**未調過的預設值**，這是它現在唯一的可信之處。
- [ ] 真正能拉開差距的是**擴大樣本**：接更多年份，或改用週頻觀測。
      CI 寬度 ±0.25 主要是 n = 77 造成的。
- [ ] **連續配重**（依 `p_bull` 線性調整防禦組權重）取代二元切換，
      理論上能降低切換造成的追蹤誤差（目前 TE 從 6.70% 升到 7.87%）。
- [ ] 若要實際使用，記得**先決定曝險目標再比較**。
      要 beta 0.93 就用槓桿版；要 beta 0.79 就用原版。
      不先固定這個，任何比較都會重蹈這次的覆轍。

---

## ⚫ 總經資料抓取：可用，但對這個問題沒有增益
