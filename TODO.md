...
來源是 `REVIEW_程式碼與數值正確性評估報告_2026-09-14.md`。**27 項全部結案**——
21 項修掉，6 項經判斷後結案為不需要修。

| 狀態 | 數量 | 編號 |
|---|---:|---|
| ✅ 已修正 | **21** | R01～R04、R06～R11、R13～R22、R27 |
| ⚪ 不需要解決（相關程式已封存） | **4** | R23、R24、R25、R26 |
| ⚪ 現階段不適用（不做組合評估） | **2** | R05、R12 |
| 🔴 未結案 | **0** | — |

「不需要解決」不等於「沒看」——四項的位置全部落在已移出主線的
`hmm_regime.py` / `fetch_macro.py` / `threshold_sweep.py`；兩項「不適用」
是研究範圍的決定，理由見下方。
...
### R05 與 R12：結案為「現階段不適用」

**原因是研究範圍的決定，不是漏掉。** 現階段**不以投資組合的方式評估因子效用**——
因子比較走的是「自有因子 vs 基準因子的 IC／ICIR」，不經過回測引擎。
R05（未來報酬篩選）與 R12（換手與成本定義）**都只影響組合績效**：

| 編號 | 影響什麼 | 對現在的評估有影響嗎 |
|---|---|---|
| R05 | 選股集合、標準化、benchmark | ❌ IC／ICIR 不選股、不建組合 |
| R12 | 換手成本、淨報酬、成本敏感度 | ❌ IC／ICIR 不扣成本 |

**要到需要比較 IR（資訊比率）時才必須修復這兩項**，因為 IR 建立在組合報酬上。
在那之前繼續投入回測系統是把工夫花在用不到的地方。

防護本身已經到位——`backtest.portfolio_returns()` 不再默默跳過缺報酬的股票，
改成直接丟 `MissingReturnError`，所以**不會有人不小心引用到被汙染的績效數字**：
...
若未來要恢復組合評估，待辦是：持股／現金帳處理停牌、恢復交易、下市／收購清算
與無法賣出（已知缺口 29 個股票月、17 檔）；以及把換手改成逐腿權重交易額。

### 多重檢定校正改走 ICIR（新增 `--deflate-icir`）

原本的 `ml_diagnose --dsr` 為了拿一個多重檢定校正，**繞道走完了整個回測引擎**
（因子 → 產業內前 10% 組合 → 扣換手成本 → 主動報酬 → Sharpe → 去膨脹），
於是同時扛上 R05 與 R12——而那兩件事跟「這個 ICIR 是不是挑出來的」毫無關係。

**ICIR 在數學形式上就是 Sharpe**（`mean/std` 的同一個估計量），去膨脹公式
吃的是「一條序列 + N 個試驗」，不在乎序列是報酬還是 IC。所以直接對 IC 序列做：

```bash
python -B src/crosssec_oos.py --deflate-icir --save logs/deflate_icir.json
```

**試驗池用的是實際搜尋歷史：1,011 個候選，含 985 個被漏斗刷掉的。**
多數研究做不到這件事，因為他們不留被拒紀錄——只剩存活者，試驗數就嚴重低估。
本專案的 `attempts/` 是只進不出，所以試驗池可以是真正的搜尋量。

⚠️ **虛無分布是最容易算錯的地方**，兩種 σ 差 2.4 倍：
...
(A) 混入了候選之間**真實的強弱差別**，拿它當虛無等於假設「所有候選都沒訊號、
觀察到的離散全是運氣」，門檻會被灌到天上去。正確的虛無是 (B)：一個沒有預測力
的因子，月 IC 均值為 0、ICIR 的抽樣誤差就是 1/√T。

結果：**24 / 26 個因子的 sub_train ICIR 超過純運氣門檻**，沒過的兩個是
F-021（0.414）與 F-008（0.398）。而且 1,011 個候選彼此高度相關（Stage 2 存在
的理由就是殺重複），有效試驗數小於 1,011 → 真實門檻比 0.428 更低，
所以這已經站在偏保守的一側。

⚠️ **test 期不做這個校正**：選取只用了 sub_train + validation，test 從未參與
挑選，沒有選擇偏誤就不需要校正。（但 test 已被歷次研究反覆檢視，不是全新
holdout——那是另一種汙染，去膨脹修不了，只能靠往前走。）

舊的 `--dsr` 保留但不預設執行，並標明它需要 R05／R12。
